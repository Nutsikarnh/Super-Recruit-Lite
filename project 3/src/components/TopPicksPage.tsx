...
function PerfectFitCard({ candidate, showResume, job }: { candidate: Candidate; showResume?: boolean; job?: JobRow }) {
  ...
  return (
    <>
      ...
      {resumeOpen && <ResumePanel onClose={() => setResumeOpen(false)} job={job} />}
...
function RunnerUpCard({ candidate, job }: { candidate: RunnerCandidate; job?: JobRow }) {
  const [status, setStatus] = useState<"sent" | "shortlisted" | "snoozed" | null>(candidate.status ?? null);
  const [showSnoozeModal, setShowSnoozeModal] = useState(false);
  const [messageOpen, setMessageOpen] = useState(false);
              ...
              <div className="mb-8">
                ...
                <div className="space-y-4">
                  {perfectFitCandidates.map((c, idx) => (
                    <PerfectFitCard key={c.id} candidate={c} showResume={idx === 0} job={selectedJob} />
                  ))}
                </div>
              </div>
              ...
              <div className="border-t border-dashed border-gray-200 pt-7">
                ...
                <div className="space-y-3.5">
                  {runnerUpCandidates.map((c) => (
                    <RunnerUpCard key={c.id} candidate={c} job={selectedJob} />
                  ))}
                </div>
              </div>
            </div>
...
const readCandidates: StatusCandidate[] = [
  {
    ...
    competencies: ["Systems Thinking", "Product Sense", "Stakeholder Alignment"],
    highlights: [
    ...
    hobbyDescription: "มองเห็น pattern และ narrative ใน visual ได้ตั้งแต่ต้น — ทำให้ออกแบบ user journey ที่มี story",
    strengthDescription: "เชื่อม design rationale กับ business direction ได้โดยไม่ต้องให้ PM translate",
    deepAnalysis: [
      { label: "ความสม่ำเสมอ", detail: "6 ปีในบริษัทเดียว — ไม่ใช่คนที่ hop งานตาม market ทั่วไป" },
      { label: "การตัดสินใจ", detail: "มี portfolio case ที่อธิบาย trade-off ได้ชัด แสดงว่า think before act" },
    ],
  ...
  },
  {
    ...
    competencies: ["Data-Informed Design", "Research Ops", "Cross-cultural UX"],
    highlights: [
    ...
    hobbyDescription: "เปิดรับ perspective ใหม่จากการเดินทาง ทำให้ research มองเห็น context ที่คนอื่นพลาด",
    strengthDescription: "นำ workshop และวิเคราะห์ data ได้ในคนเดียว ลด dependency ระหว่างทีม",
    deepAnalysis: [
      { label: "Intellectual curiosity", detail: "เลือก Podcast เป็น hobby แสดงถึงการเรียนรู้ต่อเนื่องนอกชั่วโมงงาน" },
      { label: "Cross-team impact", detail: "ทำ research แล้ว present ต่อ stakeholder ได้เอง ไม่ต้องรอ PM" },
    ],
    readAt: "เมื่อวาน",
  },
  {
    ...
    competencies: ["Motion Design", "Rapid Prototyping", "Trust-first UX"],
    highlights: [
    ...
    hobbyDescription: "สร้างงานจากศูนย์ในทั้งสองด้าน — ทำให้ approach งาน design แบบ maker ไม่ใช่แค่ executor",
    strengthDescription: "คิดนอกกรอบได้ แต่ส่งงานได้จริงตาม timeline — rare combination",
    deepAnalysis: [
      { label: "Self-sufficiency", detail: "ทำ motion prototype เองได้หมายความว่าไม่ block งานทีม dev" },
      { label: "Creative confidence", detail: "DIY hobby บ่งบอก ownership mentality ในงานที่รับผิดชอบ" },
    ],
    readAt: "2 วันที่แล้ว",
  },
  {
    ...
    competencies: ["Design Sprint", "Executive Presentation", "Workshop Facilitation"],
    highlights: [
    ...
    hobbyDescription: "Mindfulness จาก Yoga ทำให้อดทนกับ process ที่ซับซ้อนและฟัง stakeholder ได้ดี",
    strengthDescription: "นำห้องได้และเขียน documentation ชัดเจน — ทำให้ align ทีมได้โดยไม่ต้อง micromanage",
    deepAnalysis: [
      { label: "Composure under pressure", detail: "ผ่านการ present ต่อ C-level ในองค์กร regulated — ไม่หวั่นใน high-stakes meeting" },
      { label: "Clarity of thought", detail: "Writing hobby บ่งบอกถึงการ communicate ที่มีโครงสร้างและ intentional" },
    ],
    readAt: "3 วันที่แล้ว",
  },
  {
    ...
    competencies: ["Design Leadership", "Scalable Systems", "Team Mentoring"],
    highlights: [
    ...
    hobbyDescription: "Board Games ฝึก strategic planning และ reading ผู้เล่นอื่น — ทักษะที่ใช้ได้ทันทีในการ lead team",
    strengthDescription: "มองภาพใหญ่ได้และถ่ายทอดให้คนอื่น grow ได้พร้อมกัน — คุณสมบัติของ design leader ที่แท้จริง",
    deepAnalysis: [
      { label: "Long-term thinking", detail: "สร้าง design system ที่ sustain ได้ — แสดงว่าคิดถึงอนาคตทีม ไม่ใช่แค่ deadline ตอนนี้" },
      { label: "Adaptability", detail: "เล่นได้ทั้ง IC และ lead — ปรับตัวตาม team need ได้โดยไม่เสีย effectiveness" },
    ],
    readAt: "3 วันที่แล้ว",
  },
  {
    ...
    competencies: ["Visual Craft", "Communication Design", "Brand Identity"],
    highlights: [
    ...
    hobbyDescription: "สร้าง visual language ได้เองตั้งแต่ต้น — มองโลกผ่าน composition และ rhythm อยู่แล้วโดย default",
    strengthDescription: "อธิบาย design decision ได้ชัดด้วยเหตุผลเชิง communication — ไม่ใช่แค่บอกว่า 'สวยกว่า'",
    deepAnalysis: [
      { label: "Craft ownership", detail: "Illustration เป็น hobby บ่งบอกว่า invest ใน craft นอกเวลางาน" },
      { label: "Presence & energy", detail: "Dance ช่วยสร้าง confidence ใน presentation และ workshop facilitation" },
    ],
    readAt: "4 วันที่แล้ว",
  },
  {
    ...
    competencies: ["Qualitative Research", "Behavioral Analysis", "Insight Synthesis"],
    highlights: [
    ...
    hobbyDescription: "อ่านหนักและเขียนเป็นกิจวัตร — ทำให้ research report มีความลึกและ narrative ที่ stakeholder อ่านแล้วเข้าใจ",
    strengthDescription: "วิเคราะห์ data ได้ลึกและสรุป insight ออกมาเป็นภาษาที่ทีม product ใช้งานได้จริง",
    deepAnalysis: [
      { label: "Depth over breadth", detail: "Sociology background ทำให้ research มองเห็น systemic pattern ที่ surface-level interview ไม่จับได้" },
      { label: "Communication clarity", detail: "Writing hobby สะท้อน ability ใน structuring complex finding เป็น readable report" },
    ],
    readAt: "5 วันที่แล้ว",
  },
];
...
const likedCandidates: StatusCandidate[] = [
  {
    ...
    salary: "68,000 – 82,000 THB",
    skills: ["User Research", "Figma", "A/B Testing", "Analytics"],
    competencies: ["Data-Informed Design", "Stakeholder Alignment", "Workshop Facilitation"],
    highlights: [
      "5 ปีกับ multi-market product — เข้าใจ user behaviour ต่างภูมิภาคในแบบที่ไม่มี shortcut",
      "ทำ research แล้ววัดผลได้ด้วยตัวเองโดยไม่ต้องพึ่ง data team",
    ...
    strengthDescription: "นำ workshop และวิเคราะห์ data ได้ในคนเดียว ลด dependency ระหว่างทีม",
    deepAnalysis: [
      { label: "Global mindset", detail: "multi-market experience ทำให้เข้าใจ nuance ของ user ต่าง culture โดยไม่ต้องอธิบาย" },
      { label: "Self-directed", detail: "วัดผล research ด้วยตัวเองโดยไม่รอ data team — ownership สูง" },
    ],
    likedAt: "เมื่อวาน",
  ...
  },
  {
    ...
    strengthDescription: "เชื่อม design rationale กับ business direction ได้โดยไม่ต้องให้ PM translate",
    deepAnalysis: [
      { label: "Business acumen", detail: "เชื่อม design กับ business goal ได้ด้วยตนเอง — ไม่ต้องมี PM มา bridge" },
      { label: "Craft depth", detail: "ครบทั้ง system และ research — แสดงถึง T-shaped skill ที่แท้จริง" },
    ],
  ...
  },
  {
    ...
    competencies: ["Inclusive Design", "Design Ops", "Enterprise Process"],
    highlights: [
    ...
    hobbyDescription: "กีฬาที่ต้องวางแผนและอดทน — สะท้อน systematic problem-solving ที่นำมาใช้ใน design ops",
    strengthDescription: "วางโครงสร้างทีมได้และ coach คนอื่นได้ในเวลาเดียวกัน",
    deepAnalysis: [
      { label: "Rare skillset", detail: "ครอบคลุม interaction, accessibility, ops ในคนเดียว — ลด hiring needs ของทีม" },
      { label: "Resilience", detail: "Rock climbing สะท้อน mindset ของคนที่ไม่ยอมแพ้ต่อ constraint" },
    ],
  ...
  },
  {
    ...
    competencies: ["UX Strategy", "Business-aligned Design", "Service Design"],
    highlights: [
    ...
    hobbyDescription: "Endurance sports ฝึก long-term planning และ discipline — นำมาใช้ใน UX strategy ระยะยาว",
    strengthDescription: "มองภาพใหญ่ได้และแปลเป็น roadmap ที่ทีมทำตามได้จริงโดยไม่ต้องมี PM กลาง",
    deepAnalysis: [
      { label: "Hybrid thinker", detail: "MBA + UX ทำให้ design decision มี ROI story ที่ exec เข้าใจได้ทันที" },
      { label: "Stamina", detail: "Cycling เป็น long-distance sport — บ่งบอก mindset ของคนที่ทำงาน complex project ได้ถึงที่สุด" },
    ],
  ...
  },
  {
    ...
    competencies: ["Conversion-focused UX", "Mobile-first Design", "Experiment-driven Iteration"],
    highlights: [
    ...
    hobbyDescription: "มองเห็น moment ที่สำคัญผ่านเลนส์ — ทักษะเดียวกับการ identify key interaction point ใน user journey",
    strengthDescription: "ตัดสินใจจาก data ได้รวดเร็วและส่ง result ได้จริงตาม sprint goal",
    deepAnalysis: [
      { label: "Speed to insight", detail: "A/B Testing + Analytics ทำให้ iterate fast โดยมีหลักฐาน ไม่ใช่แค่ gut feeling" },
      { label: "Curiosity", detail: "Travel hobby แสดงถึงการเปิดรับ input ใหม่ — ทำให้ ideation ไม่ติดอยู่กับ pattern เดิม" },
    ],
...
const snoozedCandidates: StatusCandidate[] = [
  {
    ...
    competencies: ["Dev–Design Bridge", "User Testing", "Empathic Design"],
    highlights: [
    ...
    hobbyDescription: "Cooking ฝึกความละเอียดและ follow process — ทักษะที่ translate ได้ตรงมาสู่ design QA",
    strengthDescription: "ทำงานร่วมกับคนอื่นได้ดีและเข้าใจ user pain point ได้ลึกกว่าคนที่มีประสบการณ์มากกว่า",
    deepAnalysis: [
      { label: "Growth trajectory", detail: "ครบ feedback loop ใน 2 ปี — learning velocity สูงมาก อาจ catch up ได้เร็วกว่าที่คิด" },
      { label: "Dev fluency", detail: "IT background ช่วยลด handoff cost ระหว่าง design กับ engineering ได้จริง" },
    ],
    snoozedAt: "เมื่อวาน",
  ...
  },
  {
    ...
    competencies: ["Visual Storytelling", "Mobile UX", "Fine Arts Foundation"],
    highlights: [
    ...
    hobbyDescription: "Illustration เป็น active practice ไม่ใช่แค่ appreciation — ทำให้ visual sense อยู่ในระดับที่ train ได้ยาก",
    strengthDescription: "ส่งงานได้จริงพร้อมกับเล่าเรื่องที่ทำให้ stakeholder เข้าใจ intent ของ design",
    deepAnalysis: [
      { label: "Aesthetic calibration", detail: "Fine arts background ทำให้ visual decision มีความตั้งใจ ไม่ใช่แค่ follow trend" },
      { label: "Full ownership", detail: "ทำได้ครบในคนเดียวตั้งแต่ UX ถึง prototype — ลด iteration cycle ได้จริง" },
    ],
  ...
  },
  {
    ...
    competencies: ["Trust-first UX", "Fintech Design", "Rapid Iteration"],
    highlights: [
    ...
    hobbyDescription: "สร้างงาน creative ใน 2 medium ที่แตกต่างกัน — ทำให้ problem-solving approach หลากหลายและไม่ติด pattern เดิม",
    strengthDescription: "คิดใหม่ได้จริงภายใน constraint ที่แน่น — เหมาะกับ fintech ที่ balance innovation กับ compliance",
    deepAnalysis: [
      { label: "Potential vs. title", detail: "ครบ skills สามอย่างแต่ title ยังไม่ senior — upside สูงถ้า onboard ถูกทาง" },
      { label: "Multi-domain creativity", detail: "Photography + Music ทำให้ ideation ไม่ติดกรอบ single medium" },
    ],
  ...
  },
  {
    ...
    competencies: ["Dev–Design Bridge", "Analytical UX", "Collaborative Execution"],
    highlights: [
    ...
    hobbyDescription: "เกมฝึก systems thinking และ decision under constraint — ทักษะที่ใช้ได้ทันทีใน UX problem solving",
    strengthDescription: "วิเคราะห์ปัญหาได้ลึกพร้อมกับทำงานร่วมกับทีมได้ดี — ไม่ต้องเลือกระหว่าง quality กับ speed",
    deepAnalysis: [
      { label: "Technical bridge", detail: "CS + UX ช่วย reduce back-and-forth กับ dev team ได้อย่างมีนัยสำคัญ" },
      { label: "Strategic play", detail: "Board Games เป็น hobby ที่แสดงถึง multi-step planning และ reading ทีม" },
    ],
    snoozedAt: "1 สัปดาห์ที่แล้ว",
